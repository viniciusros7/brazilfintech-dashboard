...
# Dashboard v5 — Executive Consultant Narrative | UCLan MSc
...
T = {
    'EN': {
        ...
        'fintechs': 'Fintechs Only',
        'traditional': 'Traditional Only',
        'threshold': '⚙️ Approval Threshold (%)',
        'refresh_news': '🔄 Refresh News',
        'latest_news': '🗞️ Latest Headlines',
        'live_snapshot': '📊 Live Indicators',
        'covid_label': '🦠 COVID-19\nAcceleration',
        'segment_explain': '**Strategic insight:** The Medium Risk segment (13,010 customers · 2.9% default) is systematically underserved by traditional banks using legacy scoring. Fintechs captured this segment with AI precision.',
        'model_rationale': '### Why These Models?',
    ...
    'PT': {
        'title': '🇧🇷 Disrupção Fintech no Brasil — Briefing Executivo',
        'subtitle': 'Uma avaliação estratégica para líderes de bancos tradicionais: a ameaça fintech, as evidências e o caminho para recuperação',
        'live_pulse': '📡 Inteligência de Mercado em Tempo Real',
        'last_update': 'Última atualização',
        'tab1': '📉 Ato 1 — Mercado Perdido',
        'tab2': '🤖 Ato 2 — Motor de Crédito IA',
        'tab3': '👥 Ato 3 — Segmentos de Clientes',
        'tab4': '🔮 Ato 4 — Previsão de Mercado',
        'tab5': '🏆 Ato 5 — Por Que as Fintechs Venceram',
        'tab6': '🚀 Ato 6 — Sua Recuperação',
        'tab7': '🔬 Ato 7 — Tratamento de Dados',
        'tab8': '⚖️ Ato 8 — Ética e Risco',
        'tab9': '📰 Ato 9 — Notícias ao Vivo',
        'paradox_title': '🚨 A Crise em Números',
        'paradox_body': 'As fintechs aprovaram clientes que bancos tradicionais classificaram como <strong style="color:#E8D5B7">risco alto demais</strong> — e registraram uma <strong style="color:#4ECDC4">taxa de inadimplência menor (3,24%)</strong> que os bancos tradicionais (4,11%). No Q4 2025, atendem <strong style="color:#E8D5B7">mais de 243M contas</strong> e detêm <strong style="color:#4ECDC4">19,1% do crédito brasileiro</strong>. Cinco anos atrás, era 0,6%.',
        'approve': '✅ APROVAR',
        'conditions': '⚠️ APROVAR COM CONDIÇÕES',
        'decline': '❌ RECUSAR',
        'year_range': '📅 Intervalo de Anos',
        'show': '🔍 Mostrar',
        'all': 'Todas as Instituições',
        'fintechs': 'Apenas Fintechs',
        'traditional': 'Apenas Tradicionais',
        'threshold': '⚙️ Limite de Aprovação (%)',
        'refresh_news': '🔄 Atualizar Notícias',
        'latest_news': '🗞️ Últimas Manchetes',
        'live_snapshot': '📊 Indicadores ao Vivo',
        'covid_label': '🦠 COVID-19\nAceleração',
        'segment_explain': '**Insight estratégico:** O segmento de Risco Médio (13.010 clientes · 2,9% inadimplência) é sistematicamente sub-atendido por bancos tradicionais com scoring legado. As fintechs capturaram esse segmento com precisão de IA.',
        'model_rationale': '### Por Que Esses Modelos?',
        'rf_why': '**Random Forest vs Regressão Logística:** RF lida com relações não-lineares entre idade, renda e risco. A Regressão Logística assume linearidade — comportamento de crédito raramente é linear. RF também fornece importância de features, tornando o modelo explicável para reguladores.',
        'prophet_why': '**Prophet vs ARIMA:** Prophet lida com dados faltantes, outliers e padrões sazonais automaticamente. ARIMA exige dados estacionários e ajuste manual. Para dados com disrupção COVID, Prophet é mais robusto.',
        'kmeans_why': '**K-Means com 3 Clusters:** Método Elbow confirmou 3 como ótimo (testado 2-8). Três clusters mapeiam para segmentos reais: prime, padrão e alto risco — diretamente acionáveis para analistas de crédito.',
        'selic_explain': 'Taxa de juros de referência do Brasil (Banco Central do Brasil). Selic alta = crédito mais restrito. No nível atual, a precificação por IA dá às fintechs uma vantagem estrutural de custo sobre bancos baseados em regras.',
    }
}

# =============================================================
# CORPORATE PALETTE — v4 (less green, more boardroom)
# =============================================================
...
lang = st.sidebar.radio("🌐 Language / Idioma", ["EN", "PT"], horizontal=True)
tx = T[lang]

st.sidebar.markdown("---")
year_range = st.sidebar.slider(tx['year_range'], 2018, 2025, (2018, 2025))

inst_type = st.sidebar.radio(tx['show'], [tx['all'], tx['fintechs'], tx['traditional']])
fintech_list = ['Nubank','Inter','C6_Bank','PagBank','Neon']
trad_list    = ['Itau','Bradesco','Santander','Banco_do_Brasil','Caixa']
if inst_type == tx['fintechs']:
    selected_inst = fintech_list
elif inst_type == tx['traditional']:
    selected_inst = trad_list
else:
    selected_inst = fintech_list + trad_list

risk_threshold = st.sidebar.slider(tx['threshold'], 10, 60, 30)
...
st.markdown(f"""
<h1 style="margin-bottom:0.1rem">{tx['title']}</h1>
<p style="color:{PALETTE['muted']};font-size:1rem;margin-top:0">{tx['subtitle']}</p>
<div style="display:inline-block;background:rgba(27,79,114,0.35);border:1px solid {PALETTE['accent']};
    ...
    🕐 {tx['last_update']}: {now.strftime('%d %b %Y · %H:%M:%S')}
    </span>
...
cards = [
    ...
    ("-2.7pp", "Rate Cut by Fintechs", "IMF WP/26/15", PALETTE['fintech'], 'IMF 2026'),
]
...
for col, (val, label, change, chg_color, badge) in zip(lc, cards):
    with col:
        ...
        <div class="live-card">
            ...
            <div class="live-value" style="font-size:1.25rem">{val}</div>
            <div class="live-label">{label}</div>
            <div class="live-change" style="color:{chg_color}">{change}</div>
...
<div class="crisis-box">
    <h3 style="color:{PALETTE['trad']};margin:0 0 0.5rem 0">{tx['paradox_title']}</h3>
    <p style="color:white;font-size:1.02rem;margin:0">{tx['paradox_body']}</p>
</div>
""", unsafe_allow_html=True)
...
tab1,tab2,tab3,tab4,tab5,tab6,tab7,tab8,tab9 = st.tabs([
    tx['tab1'], tx['tab2'], tx['tab3'], tx['tab4'],
    tx['tab5'], tx['tab6'], tx['tab7'], tx['tab8'], tx['tab9']
])
...
with tab1:
    ...
    fig_dual.add_vrect(
        ...
        annotation_text=tx['covid_label'],
    ...
    with col1:
        ...
        fig_ind.add_vrect(x0="2020-01-01", x1="2021-07-01",
            fillcolor="rgba(243,156,18,0.08)", line_width=0,
            annotation_text=tx['covid_label'], annotation_position="top left",
    ...
    m4.metric("Cost of Inaction", "~30% by 2028–2029", "Prophet forecast", delta_color="inverse")
...
with tab2:
    ...
    if prob < 0.20: decision, color = tx['approve'], PALETTE['fintech']
    elif prob < risk_threshold/100: decision, color = tx['conditions'], PALETTE['warning']
    else: decision, color = tx['decline'], PALETTE['trad']

    col_r, col_g = st.columns(2)
    with col_r:
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,#0D1F2D,#1B2E3E);
                    border-radius:12px;padding:2rem;border-left:6px solid {color}">
    ...
    with col_g:
        fig_g = go.Figure(go.Indicator(
            ...
            title={'text': "Default Risk %", 'font': {'color': 'white', 'size': 14}},
            gauge={
                'axis': {'range': [0, 100], 'tickcolor': 'white', 'tickfont': {'size': 12}},
                'bar': {'color': color},
                'steps': [
            ...
            number={'suffix': '%', 'font': {'color': 'white', 'size': 44}}
        ))
...
with tab3:
    st.markdown(f"""
    <div class="insight-box">
    📣 <strong>Executive framing:</strong> K-Means clustering reveals three distinct customer groups
    in the credit data. The critical insight: the <em>Medium Risk</em> segment has a 2.9% default rate —
    lower than the average traditional bank portfolio — yet legacy scoring systematically rejects them.
    This is the <strong>untapped market fintechs monetised first</strong>.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig_pie = px.pie(seg_c, values='Count', names='Segment',
            title='Distribution by Risk Segment (28,588 real credit records)',
    ...
    with col2:
        ...
        fig_b.add_hline(y=4.11, line_dash="dot",
            line_color=PALETTE['trad'],
    ...
    st.info(tx['segment_explain'])

    # Cap income for readability, use fixed dot size, add median lines
    ...
    fig_sc = px.scatter(sample, x='person_income', y='loan_amnt',
        ...
        symbol='risk_segment',
        opacity=0.6,
        title='<b>Income vs Loan Amount — K-Means Cluster Map</b><br><sub>Each dot = real credit applicant (28,588 records, Kaggle)</sub>',
    ...
    # Add median lines per segment
    for seg, clr in cmap.items():
        seg_data = sample[sample['risk_segment'] == seg]
        if len(seg_data) > 0:
            med_inc  = seg_data['person_income'].median()
            med_loan = seg_data['loan_amnt'].median()
            fig_sc.add_annotation(
                x=med_inc, y=med_loan,
                text=f"<b>{seg}</b><br>median",
                showarrow=False,
    ...
    fig_sc.update_layout(
        plot_bgcolor='rgba(13,31,45,0.6)',
        ...
        legend=dict(orientation='h', yanchor='bottom', y=1.02, title='Risk Segment')
    )
...
with tab6:
    ...
    with sc1:
        selic_scenario = st.slider("Selic Rate Scenario (%)", 6.0, 18.0,
            float(round(selic, 1)), step=0.25,
            help="Lower Selic = cheaper credit = fintechs grow faster")
    with sc2:
        ai_invest = st.slider("Bank AI Investment (R$ Billion)", 0, 20, 3,
            help="Investment in AI credit scoring infrastructure")
    with sc3:
        digital_onboard = st.slider("Digital Onboarding (minutes)", 5, 90, 45,
            help="Current traditional bank avg: 45–60 min. Nubank: 5 min.")
    with sc4:
        partner_fintech = st.checkbox("Partner / Acquire Fintech", value=False,
            help="M&A or white-label partnership with a fintech")
    ...
    ai_dampener = min(ai_invest * 0.15, 1.8)
    onboard_dampener = max(0, (45 - digital_onboard) / 45 * 1.2)
    partner_dampener = 1.5 if partner_fintech else 0

    bank_recovery = ai_dampener + onboard_dampener + partner_dampener
    ...
    with col_chart:
        ...
        fig_sc.update_layout(
            title=f'<b>Competitive Scenario: Selic {selic_scenario:.1f}% · AI R${ai_invest}B · Onboarding {digital_onboard}min</b>',
            plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
    ...
    recs = [
        ("01", "Implement AI Credit Scoring", "URGENT",
         ...
         f"At Selic {selic:.1f}%, precision pricing is the single biggest margin lever. "
        ...
        ("03", "Digital-first Onboarding (< 10 min)", "HIGH",
         "Nubank: 4M → 131M via 5-minute mobile onboarding. "
         "Every minute above 10 loses millennial and Gen-Z applicants. "
         "Pilot: one product line, fully digital, no branch signature required.",
         PALETTE['warning'], "12–18 months", "20%"),
        ("04", f"Dynamic Selic-linked Pricing (current: {selic:.1f}%)", "MEDIUM",
         f"At Selic {selic_scenario:.1f}%, fintechs with AI pricing outcompete rule-based banks "
         f"by ~{abs(selic_scenario - 13.25) * 0.15 + 0.8:.1f}pp on credit spread. "
         f"Implement quarterly rate recalibration tied to BCB policy cycles.",
...
with tab9:
    ...
    with col1:
        ...
        if st.button(tx['refresh_news']):
            st.cache_data.clear()
            st.rerun()

    with col2:
        st.markdown(f"### {tx['live_snapshot']}")

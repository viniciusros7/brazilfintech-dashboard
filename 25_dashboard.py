...
with tab1:
    ...
    st.plotly_chart(fig_pay2, width="stretch")
    st.markdown("---")
...
with tab4:
    ...
    # Credit history correction: RF feature has inverted signal in training data.
    # Business logic: each additional year of clean history reduces risk ~0.5pp, capped at ±8pp.
    hist_adjustment = max(-0.08, min(0.08, (6 - credit_history) * 0.005))
    prob = max(0.01, min(0.99, prob + hist_adjustment))
...
with tab3:
    ...
    with br_col1:
        ...
        st.plotly_chart(fig_br_def, width="stretch")
...
with tab2:
    ...
    st.plotly_chart(fig_nu, width="stretch")
...
with tab9:
    ...
    st.markdown(f"""
    <div style="background:rgba(232,64,64,0.06);border:1px solid rgba(232,64,64,0.2);
                border-left:4px solid #E84040;border-radius:0 10px 10px 0;padding:1.2rem 1.5rem;margin-bottom:1rem">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:0.6rem">
            <span style="background:rgba(232,64,64,0.15);color:#E84040;font-size:0.75rem;font-weight:700;
                         padding:4px 12px;border-radius:20px;letter-spacing:0.08em">HIGH RISK</span>
            <span style="color:#E8EDF2;font-size:0.92rem;font-weight:600">Classification Justification</span>
        </div>
        <div style="font-size:0.82rem;color:#8AAFC4;line-height:1.7">
            <strong style="color:#E8EDF2">1. Decision impact:</strong> The model makes credit decisions that directly
            affect people's financial access — a wrong rejection means the customer loses credit and likely
            moves to a fintech permanently.<br>
            <strong style="color:#E8EDF2">2. Affected population:</strong> Millions of retail applicants across Brazil,
            including young, first-time borrowers and underbanked customers receiving automated decisions
            at scale with no human review by default.<br>
            <strong style="color:#E8EDF2">3. Reversibility:</strong> Errors are difficult to reverse — a rejected applicant
            does not return.<br><br>
            <strong style="color:#4ECDC4">Structural mitigation:</strong> BCB's Open Finance mandate (active since 2021)
            gives the model access to a customer's full financial picture across all institutions, reducing bias
            and thin-file rejections. <em>High risk, but with a credible response built into the architecture.</em>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="font-size:0.68rem;font-weight:600;color:#4ECDC4;
                letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.8rem">
        Minimum Viable Data for Safe Pilot</div>
    """, unsafe_allow_html=True)

    mvd_cols = st.columns(3)
    mvd_fields = [
        ["👤 Customer Profile",   ["Age and account type", "Account opening time", "Digital completion rate"]],
        ["📊 Credit History",     ["Credit application outcome", "Historical approval rate", "Credit score and default history"]],
        ["🏦 Open Finance (BCB)", ["Transaction frequency and channel", "Financial activity across institutions", "Cross-institutional credit behaviour"]],
    ]
    for col, (title, fields) in zip(mvd_cols, mvd_fields):
        with col:
            fields_html = "".join([
                f'<div style="padding:4px 0;border-bottom:1px solid rgba(78,205,196,0.06);'
                f'font-size:0.78rem;color:#8AAFC4">✓ {f}</div>'
                for f in fields
            ])
            st.markdown(f"""
            <div class="sust-card">
                <div style="font-size:1.2rem;margin-bottom:0.4rem">{title.split()[0]}</div>
                <div class="sust-title">{" ".join(title.split()[1:])}</div>
                {fields_html}
            </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background:rgba(15,29,46,0.6);border:1px solid {PALETTE['border']};border-radius:10px;
                padding:0.8rem 1.2rem;margin:0.5rem 0;font-size:0.82rem;color:#8AAFC4">
        <strong style="color:#E8EDF2">Pilot dataset:</strong> 28,588 records with all six field categories · AUC 0.923 ·
        <strong style="color:#E84040">Critical dependency:</strong> Open Finance — without it, the model falls back
        to legacy data and the thin-file bias that fintechs already solve comes back.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="font-size:0.68rem;font-weight:600;color:#4ECDC4;
                letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.8rem">
        Feasibility &amp; Implementation Roadmap</div>
    """, unsafe_allow_html=True)

    # Roadmap phases
    road_phases = [
        {"phase": "Pilot", "period": "Q1–Q2 2026", "status": "complete",
         "color": "#4ECDC4", "bg": "rgba(78,205,196,0.08)",
         "milestones": "RF model trained (AUC 0.923) · 28,588 records validated · Open Finance API integrated · Pre/post fairness audit passed (DIR 0.78 → 0.83)"},
        {"phase": "Deploy", "period": "Q3 2026", "status": "active",
         "color": "#E89C00", "bg": "rgba(232,156,0,0.08)",
         "milestones": "UAT in progress · Production environment configured · 30-day monitoring window planned · Rollout to 2 pilot regions"},
        {"phase": "Scale", "period": "Q4 2026 – Q1 2027", "status": "planned",
         "color": "#6B8FA8", "bg": "rgba(107,143,168,0.08)",
         "milestones": "National rollout across all regions · Full Open Finance coverage target (80%) · Precision pricing module integration · Second recertification cycle"},
        {"phase": "Optimise", "period": "Q2 2027+", "status": "planned",
         "color": "#6B8FA8", "bg": "rgba(107,143,168,0.08)",
         "milestones": "Continuous learning pipeline · Advanced segmentation · Digital onboarding overhaul · 12-month ROI checkpoint and Executive Credit Committee sign-off"},
    ]

    road_cols = st.columns(4)
    for col, rp in zip(road_cols, road_phases):
        with col:
            status_badge = f'<span style="background:{rp["bg"]};color:{rp["color"]};font-size:0.62rem;font-weight:600;padding:2px 8px;border-radius:12px">{rp["status"].upper()}</span>'
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#0F1D2E,#132640);border:1px solid rgba(78,205,196,0.08);
                        border-top:3px solid {rp['color']};border-radius:0 0 10px 10px;padding:1rem;height:100%">
                <div style="font-size:0.88rem;font-weight:700;color:#E8EDF2;margin-bottom:4px">{rp['phase']}</div>
                <div style="font-size:0.72rem;color:{rp['color']};margin-bottom:6px">{rp['period']} {status_badge}</div>
                <div style="font-size:0.75rem;color:#8AAFC4;line-height:1.6">{rp['milestones']}</div>
            </div>
            """, unsafe_allow_html=True)

    st.markdown("")

    # Operating model, resources, change management
    feas_col1, feas_col2, feas_col3 = st.columns(3)

    with feas_col1:
        st.markdown(f"""
        <div class="sust-card" style="border-top:3px solid #4ECDC4">
            <div style="font-size:1.2rem;margin-bottom:0.4rem">🏗️</div>
            <div class="sust-title">Operating Model</div>
            <div class="sust-text" style="line-height:1.7">
                <strong style="color:#E8EDF2">Hybrid governance</strong> — centralised
                AI ethics oversight at board level, operational delivery across
                Credit, Digital, and Compliance business units.<br><br>
                <strong style="color:#E8EDF2">Decision rights:</strong><br>
                ✓ Executive Credit Committee — strategic approval<br>
                ✓ CRO — halt authority (24h)<br>
                ✓ CCO — threshold transitions<br>
                ✓ Model Validation — fairness audits<br>
    ...
    with feas_col2:
        st.markdown(f"""
        <div class="sust-card" style="border-top:3px solid #EC7000">
            <div style="font-size:1.2rem;margin-bottom:0.4rem">👥</div>
            <div class="sust-title">Resources &amp; Investment</div>
            <div class="sust-text" style="line-height:1.7">
                <strong style="color:#E8EDF2">Core team:</strong><br>
                ✓ ML Engineering (model build + ops)<br>
                ✓ Data Engineering (Open Finance API)<br>
                ✓ Credit Risk (segment monitoring)<br>
                ✓ Compliance &amp; Risk (governance)<br>
                ✓ Legal (BACEN liaison)<br><br>
                <strong style="color:#E8EDF2">External:</strong><br>
                ✓ Independent bias auditor (at Validate gate)<br>
                ✓ External legal review (annual)<br><br>
                <strong style="color:#E8EDF2">Infrastructure:</strong> existing São Paulo
                data centres (78% renewable energy)
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with feas_col3:
        st.markdown(f"""
        <div class="sust-card" style="border-top:3px solid #E89C00">
            <div style="font-size:1.2rem;margin-bottom:0.4rem">🔄</div>
            <div class="sust-title">Change &amp; Adoption Plan</div>
            <div class="sust-text" style="line-height:1.7">
                <strong style="color:#E8EDF2">Challenge:</strong> Credit teams have relied on
                rule-based scoring for 20+ years. Transitioning to a model they
                cannot fully explain requires trust-building.<br><br>
                <strong style="color:#E8EDF2">Approach:</strong><br>
                ✓ Regional credit leads as model champions<br>
                ✓ SHAP explanations visible to credit officers<br>
                ✓ Phased rollout: 2 regions → national<br>
                ✓ Monthly feedback sessions (ML Eng + Credit)<br>
                ✓ Pilot Approval Rate as adoption KPI (88%)<br><br>
                <strong style="color:#E89C00">Early warning:</strong> approval rate &lt; 70%
                triggers Executive Credit Committee review
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ---- BOARD Q&A — ASSESSMENT SECTION 7 ----
    st.markdown("""
    <div style="font-size:0.68rem;font-weight:600;color:#4ECDC4;
                letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.8rem">
        Board Questions — Assessment Section 7</div>
    """, unsafe_allow_html=True)

    board_qas = [
        {
            "q": "What is the risk classification of this use case and why?",
            "short": "High risk — for three reasons: the model makes credit decisions that directly affect financial access, the affected population includes millions of vulnerable applicants, and errors are difficult to reverse.",
            "evidence": "A wrong rejection means the customer loses credit and likely moves to a fintech permanently. Young, first-time borrowers receive automated decisions at scale with no human review by default. BCB's Open Finance mandate is a structural mitigation — it gives the model a fuller financial picture, reducing thin-file bias.",
            "action": "High-risk classification drives the full six-gate governance workflow, mandatory fairness audits at Validate, and trigger-based recertification at Monitor (PSI > 0.15 or annual)."
        },
        {
            "q": "What are the top three failure modes and how will we detect them early?",
            "short": "Default rate escalation, fairness degradation, and Open Finance data coverage failure — each with a quantified early warning indicator.",
            "evidence": "Failure mode 1: model approves high-risk applicants → Default Rate Delta monitored 30-day rolling, Warning > 2.0 pp, Alert > 5.0 pp. Failure mode 2: fairness degrades silently → DIR monitored monthly, Warning < 0.79, Alert < 0.75. Failure mode 3: Open Finance coverage drops → monitored monthly, Warning < 60%, Alert < 40%.",
            "action": "Each failure mode has a pre-authorised halt trigger: Delta > 5 pp halts engine (CRO, 24h). DIR < 0.75 pauses scoring (bias audit, 5 days). Coverage < 40% pauses thin-file pipeline (Data Eng, 5 days)."
        },
        {
            "q": "Who is accountable if the AI causes harm or makes an incorrect recommendation?",
            ...
            "evidence": "Hybrid governance model with named owners at every metric. Every escalation action specifies who acts and within what time boundary. No decision in this system is unowned.",
            "action": "Quarterly review by Executive Credit Committee confirms accountability lines remain current. Three lines of defence: business unit owners (1st), Model Validation and Compliance (2nd), Internal Audit (3rd)."
        },
        {
            "q": "What is the minimum data needed to run a safe pilot?",
            "short": "Six field categories: customer profile, credit history, transaction behaviour, digital engagement, default history, and BCB Open Finance cross-institutional data.",
            "evidence": "The pilot ran on 28,588 records with these fields and achieved AUC 0.923. The critical dependency is Open Finance — without it, the model falls back to legacy data and thin-file bias returns.",
            "action": "Open Finance API connectivity and consent pipeline monitored monthly. Coverage below 60% triggers Data Engineering audit within 5 days. Below 40% pauses thin-file scoring."
        },
        {
            "q": "How will you prove value within 30–60 days?",
            "short": "Three proof points in 30 days: live default rate vs. baseline, real-world DIR readings at scale, and first explainability audit results. By day 60: early credit approval rate recovery signals.",
            "evidence": "Pilot delivered DIR 0.83, Explainability 94%, Pilot Approval 88%. The 30-day Monitor gate window confirms these hold under full production volume.",
            "action": "Monthly board report at day 30 and 60. Default Rate Delta > 2.0 pp during monitoring triggers CRO notification and segment investigation within 10 business days."
        },
    ]

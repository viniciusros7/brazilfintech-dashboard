...
with tab3:
    ...
    This is the <strong>untapped market fintechs monetised first</strong>.<br><br>
    <span style="color:{PALETTE['muted']};font-size:0.82rem">
    ⚠️ <strong>Data note:</strong> Segmentation uses the Kaggle Credit Risk Dataset (28,588 international records)
    to <em>demonstrate the AI model methodology</em>. Individual-level Brazilian credit data is not publicly
    available (LGPD protection). Brazilian market benchmarks (BCB IFData) are used for the comparison lines.
    </span>
    </div>
    """, unsafe_allow_html=True)
